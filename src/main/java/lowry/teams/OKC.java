...
public class OKC extends Team implements Roster{
	...
	public OKC(int weekNumber)
	{
		this.teamName = "OKC";
		actions = new TeamActions();
		updateWeeklyMatch(weekNumber);
	}
	public OKC(String mode, HashMap<String,Double> hmSchedule)
	{
		this.teamName = "OKC";
		actions = new TeamActions();
	...
	{
		actions.updateTeamSheet("OKC", arrPlayers);
	}
	
	public void updateWeeklyTeamSheet(HashMap<String,Double> hmSchedule)
	{
		actions.updateWeeklyTeamSheet("OKC", arrPlayers, hmSchedule);
	}

	@Override
	public void intializeRoster(String mode) {
	
		arrPlayers = actions.getTeam("OKC",mode);
